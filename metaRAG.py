...
class metaRAG_TextUpload_Request(BaseModel):
    text_content: str = Field(..., description="The plain text content to upload.")
    document_metadata: metaRAG_BaseDocument_Metadata = Field(..., 
...
class metaRAG_Ask_Request(BaseModel):
    query: str = Field(..., description="The question to ask the RAG system.")
    query_metadata_filter: metaRAG_BaseDocument_Metadata = Field(..., 
                                                        description="Metadata associated with the text, must include user_id and language.",
                                                        examples=[{
                                                            "user_id": "test_user_id",
                                                            "language": "it",
...
class metaRAG_Api_Response(BaseModel):
    status: str = "success"
...
class metaRAG_AskResponse_Data(BaseModel):
    answer: str = Field(..., description="The answer generated by the LLM.")
    retrieved_context_items: List[metaRAG_ContextItem] = Field(..., description="A list of context items used to generate the answer.")

class metaRAG_Ask_Api_Response(metaRAG_Api_Response):
    data: Optional[metaRAG_AskResponse_Data] = None
...
class metaRAG:
    ...
    def __init__(
        ...
        try:
            ...
            print(f"[metaRAG ERROR] Failed to connect to Ollama or verify models: {e}")
            raise ConnectionError(f"Could not connect to Ollama service or required models missing: {e}") from e
    ...
    def _verify_ollama_models(self):
        print("[metaRAG INFO] Verifying availability of configured Ollama models...")
        try:
        ...
        required_models = set()
        for lang_code, models in self.language_models.items():
            if "embedding" not in models or "llm" not in models:
                 raise ValueError(f"Language configuration for '{lang_code}' is missing 'embedding' or 'llm' key.")
            required_models.add(models['embedding'])
            required_models.add(models['llm'])

        missing_models = required_models - available_models
        if missing_models:
            print(f"[metaRAG ERROR] Missing required Ollama models: {', '.join(missing_models)}")
            raise ValueError(f"Missing required Ollama models: {missing_models}")
        print("[metaRAG INFO] All configured Ollama models are available.")

    def _get_models_for_language(self, lang: str) -> Tuple[str, str]:
        config = self.language_models.get(lang)
        if config:
            return config["embedding"], config["llm"]
        else:
    ...
    def _get_ollama_embedding(self, text: str, language: str, user_id_for_log: str) -> List[float]:
        try:
            embedding_model, _ = self._get_models_for_language(language)
            ...
            try: embedding_model, _ = self._get_models_for_language(language)
            except ValueError: pass
            print(f"[User: {user_id_for_log}] metaRAG ERROR: Embedding failed with model '{embedding_model}' (lang: {language}): {e}")
            raise RuntimeError(f"Ollama embedding failed for model {embedding_model} (lang: {language})") from e
    ...
    def upload_document(
        ...
        document_metadata: Union[metaRAG_BaseDocument_Metadata, Dict[str, Any]]
    ):
    ...
    def _retrieve_context(
        ...
        retrieved_items: List[metaRAG_ContextItem] = []
        try:
            ...
            if retrieved_docs and retrieved_metadatas and retrieved_distances and len(retrieved_docs) == len(retrieved_metadatas) == len(retrieved_distances):
                 ...
                 for doc, meta, dist in zip(retrieved_docs, retrieved_metadatas, retrieved_distances):
                    retrieved_items.append(metaRAG_ContextItem(text=doc, metadata=meta, distance=dist))
    ...
    def ask(
        self,
        query: str,
        query_metadata_filter: Union[metaRAG_BaseDocument_Metadata, Dict[str, Any]],
        n_results_for_context: int = 3
        ) -> Tuple[str, List[metaRAG_ContextItem]]:
        ...
        retrieved_context_items_list: List[metaRAG_ContextItem] = self._retrieve_context(
            query,
        ...
        _, llm_model = self._get_models_for_language(language)
        print(f"[User: {user_id}] metaRAG INFO: Using LLM '{llm_model}' for language '{language}'.")
...
@app.get("/health/", response_model=metaRAG_Api_Response, tags=["Utility"])
async def health_check():
    ...
    try:
        ...
        return metaRAG_Api_Response(message="metaRAG API is healthy.")
    except Exception as e:
        print(f"Health check failed: {e}")
        raise HTTPException(status_code=503, detail=f"Service unhealthy: {e}")

@app.post("/upload/file/", response_model=metaRAG_Api_Response, tags=["Upload"])
async def upload_file_endpoint(
    ...
    try:
        ...
        doc_metadata = metaRAG_BaseDocument_Metadata(**metadata_dict)
    ...
    try:
        ...
        source_name = rag_system_instance.upload_document(
            ...
            document_metadata=doc_metadata # Pass the Pydantic model instance
        )
        return metaRAG_Api_Response(message=f"File '{source_name}' uploaded and processed successfully.")
    except ValueError as ve:
...
@app.post("/upload/text/", response_model=metaRAG_Api_Response, tags=["Upload"])
async def upload_text_endpoint(request: metaRAG_TextUpload_Request):
    if rag_system_instance is None:
        raise HTTPException(status_code=503, detail="metaRAG not available. Service may not have started correctly.")
    try:
        ...
        return metaRAG_Api_Response(message=f"Text content from source '{source_name}' uploaded and processed successfully.")
    except ValueError as ve:
        raise HTTPException(status_code=400, detail=str(ve))
    except RuntimeError as re:
        raise HTTPException(status_code=500, detail=str(re))
...
@app.post("/ask/", response_model=metaRAG_Ask_Api_Response, tags=["Query"])
async def ask_endpoint(request: metaRAG_Ask_Request):
    ...
    try:
        ...
        return metaRAG_Ask_Api_Response(
            message="Query processed successfully.",
            data=metaRAG_AskResponse_Data(
                answer=answer,
